...
1. **Retrieval audit.** Review `knowledge/_ksearch-log.tsv` (timestamp, query, top hit, total match count). Add real misses and expected notes to the [evaluation fixture](../retrieval-evaluation.md) before making changes. Run `python3 scripts/ksearch-eval.py knowledge/evals/cases.jsonl --root knowledge`; distinguish absent knowledge, vocabulary mismatch, wrong rank, and budget omissions. Patch relevant titles/TL;DRs when evidence supports it. Keep held-out queries unchanged for comparison; starter cases alone cannot establish project quality.
...
4. **Size/structure audit.** Run `python3 scripts/kbase-doctor.py --context-only` to inspect startup material and skill descriptions. Files over ~400 lines are split candidates. Archive superseded facts with a dated note; low query frequency alone does not make a critical rule obsolete.
