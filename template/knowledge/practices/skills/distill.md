...
> **TL;DR:** Turn a SCAFFOLDED knowledge base into a VERIFIED one by executing the bootstrap brief: verify the evidence pack, write architecture/pitfalls/operations with per-claim labels, fill TODO markers, flip the status and delete the brief in one gate-checked commit.
...
**Preconditions:** you have repo read access and can commit. The KB is untrustworthy until this pass completes — do not cite it meanwhile.

## Steps

1. **Read `knowledge/_bootstrap-brief.md` fully.** Its evidence pack is mechanical and can be wrong. Before building on any line (stack claim, LOC count, entry point, history theme), verify it against the source with `rg` or file reads. Wrong evidence gets corrected in your output — never propagated.
...
6. **Record first lessons** in `learning/lessons-log.md` (append-only, dated, ✅/❓ — format in that file) if the history taught anything durable.
7. **Flip and delete in ONE commit:**
   ...
   - commit message: `knowledge: distillation pass — KB VERIFIED`
   - `scripts/knowledge-gate.py` verifies the named commit exists and that VERIFIED and the brief never coexist; if the gate blocks, fix the state, don't work around it.
